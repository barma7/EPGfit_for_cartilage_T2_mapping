function [T2fit, B1fit, Data_est, tElapsed] = dictionary_matching_dot(LUT, Dict, Data)
...
for b = 1:Nb_blocks
    ...
    if (b == Nb_blocks && stop_indx>Nb_tests)
        ...
        for j=1:length(maxDot_array) 
            ...
            T2fit(indx) = LUT(Mtch_indx,1);
            B1fit(indx) = LUT(Mtch_indx,2);
            Data_est(indx, :) = Dict(Mtch_indx, :);
