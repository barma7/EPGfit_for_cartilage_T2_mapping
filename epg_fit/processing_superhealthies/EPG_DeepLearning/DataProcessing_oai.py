...
home_data_folder = "/bmrNAS/people/barma7/Lab-work/Projects/OAI_T2mapping/repository_JMRI/DATA/superhealthies"
...
sv_root = "elastix_registration/DL"
sv_fldr = copy(w_run)
...
# Data Processing
list_sub_id, list_time, list_status, list_msg, list_time_id = [], [], [], [], []
strt_tot = time.time()
...
for fldr in list_participant_folders:
    ...
    list_subfolders = glob.glob(os.path.join(fldr, "*"))

    for sub_fldr in list_subfolders:
        time_id = os.path.basename(sub_fldr)
        print(f'Processing time point {time_id}')
        list_time_id.append(time_id)

        # If data exists in folder process data, otherwise skip
        if os.path.exists(os.path.join(sub_fldr, nifti_name)):
            SvFldr = os.path.join(home_save_path, sub_id, time_id, sv_name)
            Path(SvFldr).mkdir(parents=True, exist_ok=True)

            # Load Data
            nr = dm.NiftiReader()
            data_nifti = nr.load(os.path.join(sub_fldr, nifti_name))
            mask_nifti = nr.load(os.path.join(sub_fldr, mask_name))
...
df = pd.DataFrame(list(zip(list_sub_id, list_time_id, list_status, list_time, list_msg)), 
                  columns=['sub_id', 'time_id', 'status', 'processing time', 'message'])
df.to_csv(os.path.join(home_save_path, 'processing_log_DL.csv'), index=False)
